...
* 📝 `Phase2 Task1.ipynb` - (Bank Marketing XAI) Classification with Random Forests and SHAP interpretability.
* 📝 `Phase2 Task2.ipynb` - (Customer Segmentation) Unsupervised K-Means clustering and PCA dimensionality reduction.
...
* **Objective:** Predict if a customer will open a term deposit and decode model parameters using XAI.
* **Architecture:** Random Forest Classifier & Logistic Regression baseline (`saga` solver).
* **Interpretability Framework:** Integrated **SHAP (SHapley Additive exPlanations)** to map explicit local and global feature impact parameters, proving that telemarketing `duration` and historical `poutcome_success` are the strongest drivers for customer conversion.
...
* **Workflow:** Scaled raw columns using `StandardScaler` to remove distance skewing. Applied the **Elbow Method** to calculate the Within-Cluster Sum of Squares (WCSS), identifying $K=5$ as the optimal grouping point.
* **Visualization:** Implemented **Principal Component Analysis (PCA)** to reduce the 3D metric landscape down to 2D space for cluster mapping.
